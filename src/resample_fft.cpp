...
List resample_mvfft( const arma::cx_mat& fftz,
  ...
  arma::cx_double fdc1 = arma::cx_double(0.0, 0.0);
  arma::cx_double fdc2 = arma::cx_double(0.0, 0.0);
...
//' @export
// [[Rcpp::export]]
List resample_mvfft_parallel(const arma::cx_mat& fftz,
                           const arma::ivec& tapers,
                           bool verbose = true,
                           const bool dbl = true,
                           const int tapcap = 12000) {
  ...
  RcppParallel::parallelFor(0, nhalf, calc_resample_fft);
  ...
  // return list to match previous function definition
  return Rcpp::List::create(
    Named("freq.inds") = arma::regspace<arma::ivec>(1, nhalf),
    Named("k.capped") = taper_vec(arma::span(0, nhalf-1)),
    Named("psd") = psd
...
taps <- ceiling(runif(n./nc,10,5000))
...
microbenchmark(
  ...
  rsz4 <- resample_mvfft_parallel(fftz, taps, verbose = FALSE),
  times = 1
)
